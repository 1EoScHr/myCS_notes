...
**光栅化**不能很好的解决**全局性阴影**问题，譬如软阴影（离物体远，影子会变模糊）、Glossy reflection（有部分高光反射，但整体粗糙，类似古代铜镜）、间接光照（理解为光线进入眼睛前经过多次反射），在这些情况下使用光栅化效率会非常低。因此**光栅化就是快速的近似**，在real-time情境下多用。  
